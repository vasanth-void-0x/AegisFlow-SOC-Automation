param(
  ...
  [ValidateSet("low","medium","high","critical")][string]$AutomationMinSeverity = "medium"
)
...
$Config = @{ api_url=$ApiUrl.TrimEnd('/'); api_key=$ApiKey; source_name=$SourceName; profile=$Profile; n8n_webhook_url=$N8nWebhookUrl; automation_min_severity=$AutomationMinSeverity; poll_seconds=15; state_file=(Join-Path $InstallDir "state.json"); spool_file=(Join-Path $InstallDir "spool.json") }
