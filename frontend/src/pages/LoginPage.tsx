...
export function LoginPage() {
  ...
  return <main className="login-page">
    ...
    <form className="login-card" onSubmit={submit}>
      ...
      <label><span>USERNAME</span><div className="login-input"><i aria-hidden="true">01</i><input autoComplete="username" value={username} onChange={event=>setUsername(event.target.value)} placeholder="Enter operator ID" required/></div></label>
      <label><span>PASSWORD</span><div className="login-input"><i aria-hidden="true">••</i><input type="password" autoComplete="current-password" value={password} onChange={event=>setPassword(event.target.value)} placeholder="Enter secure password" required minLength={12}/></div></label>
