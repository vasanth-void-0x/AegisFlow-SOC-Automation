...
export function LoginPage() {
  ...
  return <main className="login-page">
    ...
    <form className="login-card" onSubmit={submit} autoComplete="off">
      ...
      <label><span>USERNAME</span><div className="login-input"><i aria-hidden="true">01</i><input name="blueorch_operator" autoComplete="off" value={username} onChange={event=>setUsername(event.target.value)} required/></div></label>
      <label><span>PASSWORD</span><div className="login-input"><i aria-hidden="true">••</i><input name="blueorch_access_code" type="password" autoComplete="new-password" value={password} onChange={event=>setPassword(event.target.value)} required minLength={12}/></div></label>
